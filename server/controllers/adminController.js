...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const revenueGrowth =
      parseFloat(monthlyRevenue[0].total) -
      parseFloat(lastMonthRevenue[0].total);
...
export const updateProduct = async (req, res) => {
  try {
    ...
    const { updateFields, values } = await buildUpdateQuery(
      allowedFields,
      req.body,
      req.file,
      "products"
...
export const createCategory = async (req, res) => {
  try {
    const { name, slug, headline, description, accent_gradient, hero_tagline } =
      req.body;
    ...
    await pool.execute(
      "INSERT INTO categories (name, slug, headline, description, accent_gradient, hero_tagline, image_url) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [
        name,
        categorySlug,
        headline || "",
...
export const updateCategory = async (req, res) => {
  try {
    ...
    const oldImagePath =
      oldCategories.length > 0 ? oldCategories[0].image_url : null;
    ...
    return sendSuccess(res, {
      ...
      results: blogs.map((blog) => ({
        ...
        content_blocks: blog.content_blocks
          ? JSON.parse(blog.content_blocks)
          : null,
...
export const getBlog = async (req, res) => {
  try {
    ...
    return sendSuccess(res, {
      ...
      author_name: blog.author_name,
      excerpt:
        blog.excerpt || (blog.content ? blog.content.substring(0, 200) : null),
      date: blog.date || blog.created_at,
      featured_image_url: blog.featured_image
...
export const createBlog = async (req, res) => {
  try {
    ...
    if (req.files && req.files.thumbnail) {
      thumbnailName = await processImage(
...
export const updateBlog = async (req, res) => {
  try {
    ...
    const oldImagePath =
      oldBlogs.length > 0 ? oldBlogs[0].featured_image : null;
    ...
    return sendSuccess(
      res,
      {
        ...
        photo: updated[0].photo
          ? getImageUrl(req, updated[0].photo, "admins")
          : null,
