        ...
        <div className="flex items-center justify-between mb-6 sm:mb-10">
          <div className="flex items-center space-x-3">
            <Link to="/" className="inline-flex items-center text-slate-600 hover:text-slate-900 transition-colors">
              ...
              <span className="ml-2 text-sm sm:text-base">Continue Shopping</span>
            </Link>
          </div>
          <h1 className="text-xl sm:text-2xl font-semibold tracking-tight text-slate-900">Your Cart</h1>
        </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.length === 0 && (
              <div className="p-8 text-center rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <Link to="/product" className="inline-flex items-center px-4 py-2 rounded-lg bg-gradient-to-r from-myura-pink-500 to-myura-rose-500 text-white hover:from-myura-pink-600 hover:to-myura-rose-600 transition-all duration-200 shadow-sm">Browse Products</Link>
              </div>
            )}
            {items.map((item) => (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      ...
                      {item.variant && <p className="text-xs sm:text-sm text-slate-500">{item.variant}</p>}
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <p className="text-sm sm:text-base font-semibold text-slate-900">₹ {item.price * item.qty}</p>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-slate-900">{shipping === 0 ? 'Free' : `₹ ${shipping}`}</span>
                </div>
              ...
              <button disabled={items.length === 0} className="w-full mt-5 inline-flex justify-center items-center px-4 py-3 rounded-xl bg-gradient-to-r from-myura-pink-500 to-myura-rose-500 text-white font-medium hover:from-myura-pink-600 hover:to-myura-rose-600 transition-all duration-200 shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">Proceed to Checkout</button>
              <p className="mt-3 text-xs text-slate-500 text-center">Free shipping on orders over ₹799</p>
            </div>
          </aside>
        </div>
      </div>
